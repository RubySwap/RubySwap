...
// ApprenticeJeweler is the chef of new tokens. He can make yummy food and he is a fair guy as well as MasterJeweler.
contract ApprenticeJeweler {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many GEM tokens the user has provided.
        ...
        //
        // Whenever a user deposits or withdraws GEM tokens to a pool. Here's what happens:
    ...
    // Info of each user that stakes Gem tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = gem.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 gemSupply = gem.balanceOf(address(this));
        if (gemSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(gemSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit Gem tokens to ApprenticeJeweler for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        gem.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Gem tokens from ApprenticeJeweler.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        gem.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        gem.safeTransfer(address(msg.sender), user.amount);
